...
messageterminal.prototype.sendMessage = function(obj, to) {
  ...
  for (var i=0; i<recipients; i++) {
    var m = new message(obj, recipients[i], this.terminalId);
    dsm.messagebroker.putMessage(m);
  }
};
...
function identityprovider() {
  if (!simpleStorage.nodeId)
    simpleStorage.nodeId = this.generateNodeId();
  console.log("nodeId: " + simpleStorage.nodeId);
}

identityprovider.prototype.nodeId = function() {
  return "" + dsm.socketprovider.getListeningPort();
  return "" + simpleStorage.nodeId;
};
...
identityprovider.prototype.generateNodeId = function() {
  ...
  for (var i=0; i<bytes; i++) 
    id += Math.floor(Math.random() * 256).toString(16);
  return id;
};
...
controller.prototype.getNodeId = function() {
    return dsm.identityprovider.nodeId();
};
